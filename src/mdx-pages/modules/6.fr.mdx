...
- b) [🍬 Pour se rafraichir la mémoire un peu plus (pas obligatoire)](#b)
- c) [Premier accès à la DB](#c)
- d) [Création d'une API REST avec MongoDB](#d)
- e) [Gestion des erreurs](#e)
- f) [🍬 Pour aller plus loin : Update dans la DB](#f)
- g) [A retenir](#g)
...
# <a id="b"></a> b) 🍬 Pour se rafraichir la mémoire un peu plus (optionnel)
...
# <a id="c"></a>c) Premier accès à la DB
...
Pour faire l'exercices 12, nous vous invitons à créer un nouveau projet, par exemple dans le dossier `/exercises/module6/ex3.12` en lançant la commande `npm init` dans le dossier du projet. Une fois le projet créé, il faut y installer les dépendances (mongoose...)
...
# <a id="d"></a>d) Création d'une API REST avec MongoDB
...
# <a id="f"></a>f) 🍬 Pour aller plus loin : Update dans la DB (optionnel)
...
Dans l'exercices 3.16, gérez à présent tous les cas d'erreur de votre application.
...
# <a id="h"></a> h) 🍬 Solution des exercices
Une solution pour les exercices de ce module se trouvent ici : [module6](https://gitlab.vinci.be/6i3-web3/2022-2023/module6.git).
