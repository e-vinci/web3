...
- a) [Rappel : création d'une API REST](#a)
- b) [Premier accès à la DB](#b)
- c) [Création d'une API REST avec MongoDB](#c)
- d) [🍬 Pour aller plus loin : Update dans la DB](#d)
- e) [A retenir](#e)
...
# <a id="a"></a>a) Rappel : création d'une API REST

Dans le cours de Web 2, vous avez appris comment faire une API REST avec Node.js. Nous vous invitons à lire [Node.js and Express](https://fullstackopen.com/en/part3/node_js_and_express) [[R.1]](/references/#r1) pour vous raffraichir la mémoire. Si vous souhaitez faire les exercices également, libre à vous, mais attention que la vraie matière de cette séance est plutôt portée sur MongoDB. Ces exercices sont considérés comme connus.
...
# <a id="b"></a>b) Premier accès à la DB
...
# <a id="c"></a>c) Création d'une API REST avec MongoDB
...
Repartez de la solution de la partie a) disponible sur Gitlab ici : [Solution module 6 rappels](https://gitlab.vinci.be/6i3-web3/2022-2023/module6-rappels). Veillez à refactorer le code pour utiliser une structure adéquate et maintenable.
...
# <a id="d"></a>d) 🍬 Pour aller plus loin : Update dans la DB (Pas évalué)

Faites les exercices 3.16, 3.17 et 3.18 de la séance lue précédemment : [Saving data to MongoDB](https://fullstackopen.com/en/part3/saving_data_to_mongo_db#exercises-3-15-3-18)
...
* Mongoose va nous connecter à la DB, et nous proposer de créer des Modèles, sur base de Schémas. Ces modèles sont fonctions "constructeur", ce qui nous donne des objets JS qui ont des méthodes spécifiques, notamment une méthode "save" qui permet d'ajouter l'objet créé en DB.
* On n'est pas obligés d'utiliser Mongoose et toutes les facilités associées. Il existe une driver Node.js officiel "de base" qui permet de faire toutes les opérations nécessaires sur la DB. Il ne fournit par constre pas les systèmes de schéma, de modèle... Lorsque ces éléments ne vous sont pas utiles, préférez le driver par défaut.
...
* Le code qui nous permet de nous connecter à la DB, les shémas, les modèles...  doivent être mis dans un module séparé.