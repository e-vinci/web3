...
Dans les applications que nous développons dans ce cours, l'architecture est complètement différente. Nous développons des **Single-Page-Applications** (**SPA**), pour changer de page :
...
Si nécessaire, vous pouvez trouver le code associé à ce tutoriel ici : [router-starter](https://github.com/e-vinci/ts-demos/tree/main/front/routing).
...
Ensuite, commençons par changer les "inputs" de notre composant **`AddPizzaPage`**, en utilisant les composants "**Input**" de **Ant Design** : [Input](https://ant.design/components/input/) [[R.9]](/references/#r9).  
