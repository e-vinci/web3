...
- La philosophie de GraphQL est fort différente du REST. En REST, on reçoit toute la ressource, même ce dont on a pas besoin, et si on veut des éléments "impriqués", il faut les demander séparément, ou faire une requête spécifique. En GraphQL, on reçoit uniquement ce qu'on demande, et on peut demander directement des éléments imbriqués.
- Lorsqu'on fait une Query GraphQL, par défaut on ne reçoit rien, il faut explicitement demander ce qu'on veut reçevoir.
...
- Appolo Server s'occupe en grande partie de la gestion des erreurs, grâce aux schémas. Si le client ne restpecte pas le schéma, ou le format des mutations, Appolo est capable de le détecter seul. On ne doit donc s'occuper que de la gestion des erreurs "business".
