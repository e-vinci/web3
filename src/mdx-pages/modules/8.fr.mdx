...
- La philosophie de GraphQL est fort différente du REST. Au lieu d'écrire une fonction JS qui sera exécutée pour chaque requête, on écrit plutôt des resolvers pour chaque field de chaque ressource, et des mutations. Ceci nous permet, du côté client de faire des requêtes "personnalisées" en fonction des besoins (on choisit ce qu'on récupère)
